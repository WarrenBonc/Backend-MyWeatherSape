...
router.get("/adult", authenticateToken, async (req, res) => {
  const clothingItems = await User.findById(req.user.id).populate(
    "clothingItems"
  );
...
router.delete("/delete/:id", authenticateToken, async (req, res) => {
  ...
  if (item.userId.toString() !== req.user.id) {
    return res
      .status(403)
      .json({ message: "Non autorisé à supprimer ce vêtement" });
  }
...
router.put("/edit/:id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur lors de la mise à jour du vêtement", error });
  }
});
